...
def accuracy(typed, source):
    ...
    max_length = max(len(typed_words), len(source_words))
...
def report_progress(typed, source, user_id, upload):
    ...
    adict = {'id': user_id, 'progress': progress}
    upload(adict)
    return progress
    # END PROBLEM 8
...
def fastest_words(words_and_times):
    ...
    for word_index in word_indices:
        ...
        for player_num in player_indices:
            current_time = get_time(times, player_num, word_index)
            if current_time < fastest_time:
                fastest_player = player_num
                fastest_time = current_time

        fastest[fastest_player] = fastest[fastest_player] + [words[word_index]]
    return fastest
    # END PROBLEM 10
